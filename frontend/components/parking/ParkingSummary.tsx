...
export function ParkingSummary({ status, slots }: ParkingSummaryProps) {
  ...
  return (
    <div className="stats-row parking-summary" aria-label="Parking status summary">
      <div><span className="stat-dot available" /><span><b>{status.available}</b><small>Available</small></span></div>
      <div><span className="stat-dot reserved" /><span><b>{status.reserved}</b><small>Reserved</small></span></div>
      <div><span className="stat-dot occupied" /><span><b>{status.occupied}</b><small>Occupied</small></span></div>
      <div><span className="stat-dot ev" /><span><b>{availableEv}</b><small>EV available</small></span></div>
