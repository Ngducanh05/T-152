...
async function installAuthMocks(page: Page) {
  ...
  await page.route("http://supabase.parksmart.test/**", async (route) => {
    ...
    if (url.pathname === "/auth/v1/token") {
      ...
      if (grantType === "refresh_token") {
        const account = body?.refresh_token
  ...
  await page.route("http://api.parksmart.test/**", async (route) => {
    const request = route.request();
    if (request.method() === "OPTIONS") {
      await route.fulfill({ status: 204, headers: responseHeaders() });
      return;
    ...
    if (url.pathname === "/api/v1/auth/me") {
      if (!account) {
        await fulfillJson(route, failure("INVALID_TOKEN", "Token is invalid."), 401);
        return;
      }
      await fulfillJson(
        route,
        success({
          id: account.id,
          email: account.email,
          full_name: account.role === "admin" ? "Admin E2E" : "User E2E",
          role: account.role,
          parking_user_id: account.parking_user_id,
          default_vehicle_id: account.default_vehicle_id,
        }),
      );
      return;
    }
    ...
    if (!account) {
      await fulfillJson(route, failure("AUTH_REQUIRED", "Authentication required."), 401);
      return;
    }

    if (url.pathname === "/api/v1/parking/map") {
      await fulfillJson(
        route,
        success({ floor_id: "F1", nodes: [], edges: [], slots: [] }),
      );
      return;
    }
    if (url.pathname === "/api/v1/parking/slots") {
      await fulfillJson(route, success([]));
      return;
    }
    if (url.pathname === "/api/v1/parking/status") {
      await fulfillJson(
        route,
        success({
          total: 40,
    ...
    if (
      url.pathname === "/api/v1/locations/current" ||
      url.pathname === "/api/v1/reservations/active" ||
      url.pathname === "/api/v1/sessions/active"
    ) {
      await fulfillJson(route, failure("NOT_FOUND", "Not found."), 404);
      return;
    }
...
test("guest cannot access user or admin routes", async ({ page }) => {
  await page.goto("/");
...
test("invalid login shows a safe error without role selection", async ({ page }) => {
  await page.goto("/login");
...
test("user login routes to chat, survives reload, blocks admin, and logs out", async ({ page }) => {
...
test("admin login routes to dashboard and redirects away from user route", async ({ page }) => {
